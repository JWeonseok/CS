...
그래프(`Graph`)는 **연결되어 있는 객체 간의 관계**를 표현하는 자료구조이다. 추가적으로 트리는 사이클이 없는 그래프이다. 그래프는 정점(`Vertex`)와 간선(`Edge`)로 구성되어 있다. 그래프 G는 (V, E)로 표현한다.
...
- **차수**(`Degree`)
  ...
  무방향 그래프에서 **각 정점에 연결된 간선의 개수**를 차수라고 한다. 방향 그래프의 경우 각 정점에서 나가는 간선의 개수를 Outdegree, 들어오는 간선의 개수를 Indegree라고 한다.
...
가중치 그래프(`Weighted Graph`)는 간선에 비용(`cost`)이나 가중치(`weight`)가 할당된 그래프를 의미한다.

부분 그래프(`Sub Graph`)는 그래프 G의 정점 집합 V(G)와 간선 집합 E(G)의 부분 집합으로 이루어진 그래프를 의미한다.



### 그래프의 경로

그래프에서 한 정점 s로부터 정점 e까지 가는 것을 경로(`path`)라고 하고 경로 중에 반복되는 간선이 없는 경로를 단순 경로(`simple path`)라고 한다. 사이클(`cycle`)은 시작 정점과 종료 정점이 동일한 경로를 의미한다.
...
- 인접 행렬 (`Adjacent Matrix`)
  ...
  그래프를 인접 행렬로 표현할 때, 그래프에 간선 (i, j)가 존재할 경우 i행 j열에 1, 그렇지 않을 경우 0의 값을 가진다. 가중치 그래프의 경우 1이 아닌 가중치의 값을 가진다.

  인접 행렬에서 연결 여부를 파악할 때 간선 개수와는 무관하게 시간 복잡도는 O($$V^2$$)를 가지므로 간선의 개수가 많은 밀집 그래프(`Dense Graph`)에 이용하기 적합하다.

- 인접 리스트 (`Adjacent List`)

  각 정점에 연결된 정점들을 연결리스트로 표현한 것이 인접 리스트이다. 인접 행렬과는 다르게 시간 복잡도를 O(E + V)를 가지므로 간선의 개수가 영향을 미친다. 따라서 간선의 개수가 적은 희소 그래프(`Sparse Graph`)를 표현하는 것에 적합하다.



### 그래프 탐색

그래프는 정점과 간선의 연결의 규칙이 존재하지 않기 때문에 탐색 방법이 필요하다. 따라서 그래프의 모든 정점을 탐색하는 두 가지 알고리즘이 존재한다.

- **깊이 우선 탐색(`DFS : Depth First Search`)**

  그래프에서 한 정점에서 연결된 다른 한 정점으로, 한 방향으로 계속해서 탐색해 나간다. 계속해서 탐색을 진행하다가 더 이상 연결된 정점이 없다면, 이전 정점으로 돌아와서 연결된 다른 정점을 파악하고 그 정점으로 다시 탐색을 진행한다.

  이렇게 **이전 단계로 되돌아온다**는 특징 때문에 DFS를 구현하기 위해서 **Stack**의 자료구조를 사용한다. 시간 복잡도는 **O(V + E), 즉 정점의 개수 + 간선의 개수**이다.

- **너비 우선 탐색(`BFS : Breadth-First Search`)**

  그래프에서 한 정점으로부터 연결된 모든 정점들을 방문하고, 멀리 떨어져 있는 정점은 나중에 방문하는 탐색 방법이다. BFS를 구현하기 위한 자료구조는 **FIFO 특징을 가지는 Queue**이다. 먼저 한 정점으로부터 인접한 정점들을 큐에 삽입한다. 그리고 큐에서 하나씩 삭제하면서 삭제되는 정점과 인접한 정점들을 다시 큐에 삽입한다. 즉, 방문한 정점들 순서대로 큐에 저장하는 것이다. 시간 복잡도는 DFS와 동일한 O(V + E)를 가지며 **BFS로 구한 경로는 최단 경로라는 특징을 가진다.**



### Minimum Spanning Tree

우선 신장 트리(`Spanning Tree`)에 대해서 알아야 한다. 신장 트리는 **그래프의 모든 정점을 포함하며 사이클이 존재하지 않게 연결되는 것**을 의미한다. 따라서 n개의 정점을 가지는 그래프의 신장 트리는 n-1개의 간선을 가진다.

이러한 신장 트리의 간선들의 가중치가 최소가 되는 것을 **최소 신장 트리**(`Minimum Spanning Tree`)라고 한다.

최소 신장 트리를 구하는 다음의 두 가지 알고리즘이 존재한다.

#### Kruskal Algorithm

크루스칼 알고리즘(`Kruskal Algorithm`)의 기저에는 그리디 방법(`Greedy Method`)이 깔려있다. 그리디 방법은 **각 단계에서 최선의 답을 선택하는 과정을 반복함으로써 최종적인 해답에 도달**하는 방법이다. 크루스칼 알고리즘은 다음과 같다.

1. 그래프의 간선들을 가중치의 오름차순으로 정렬한다.
2. 정렬된 간선 중에서 사이클을 형성하지 않은 간선을 현재의 MST 집합에 추가한다.
3. 만약 사이클을 형성하면 해당 간선은 제외한다.

그런데 위와 같은 크루스칼 알고리즘을 구현할 때 **어떻게 사이클 존재 여부를 판별할까?**

사이클의 존재 여부를 판별하기 위해서는 **Union-Find** 알고리즘을 알아야 한다. Union-Find 알고리즘은 Disjoint-Set 알고리즘, 즉 **임의의 두 원소가 같은 집합에 속하는 지 판별하는 방법**이다. 따라서 해당 정점들이 같은 집합에 속한다면 사이클을 형성하는 것이므로, 해당 간선은 제외해야 한다.

크루스칼 알고리즘의 시간 복잡도는 간선의 정렬 O(E log E), 사이클 생성 여부를 판단하는 Union-Find O(E + V log V)로 전체 시간 복잡도는 **O(E log E)**가 된다.

#### Prim Algorithm

프림 알고리즘(`Prim Algorithm`)은 **시작 정점에서부터 출발하여 신장 트리 집합을 단계적으로 확장해나가는** 방법이다. 우선 시작 정점만이 신장 트리 집합에 포함되어 있다. 그리고 해당 정점의 인접한 정점 중에서 **최소 간선으로 연결된 정점을 선택하여 집합에 추가한다.** 이 과정이 n-1개의 간선을 가질 때까지 반복한다. 프림 알고리즘의 시간 복잡도는 O(E log V)이다.
